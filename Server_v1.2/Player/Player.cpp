...
void Player::slotReadFromAppToServ(QDataStream& dataBlock)
{
    ...
    switch (typeSignal)
    {
        ...
        case EthernetSignals::ClientSignal::ApplicationSignal::MessageSignal       :
        {
            ...
            if(canal == Player::Canal::GameCanal)
            {
                emit this->messageSignalUserSendMessagePlayers(this->m_ID, message);
            }
            else
            {
                emit this->messageSignalUserSendMessageOnCanal(this->m_ID, canal, message);
            }
...
void Player::messageSlotReceiveMessageFromServer(const QString& message)
{
    QByteArray dataBlock;
    QDataStream outStream(&dataBlock, QIODevice::OpenModeFlag::WriteOnly);
    outStream.setVersion(QDataStream::Version::Qt_5_14);
    outStream << qint16(0)
              << EthernetSignals::ServerSignal::TypeSignal::FromServToApp
              << EthernetSignals::ServerSignal::ServerSignal::ServerMessageSignal
              << message;
...
void Player::messageSlotReceiveMessageFromInstance(quint16 userID, const QString& message)
{
    ...
    outStream << qint16(0)
              << EthernetSignals::ServerSignal::TypeSignal::FromServToApp
              << EthernetSignals::ServerSignal::ServerSignal::UserMessageSignal
              << userID
              << Canal::GameCanal
              << message;
...
void Player::messageSlotReceiveMessageFromUser(quint16 userID, Player::Canal chatCanal, const QString& message)
{
    QByteArray dataBlock;
    QDataStream outStream(&dataBlock, QIODevice::OpenModeFlag::WriteOnly);
    outStream.setVersion(QDataStream::Version::Qt_5_14);
